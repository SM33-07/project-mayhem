...
export async function GET(request: NextRequest) {
  try {
    ...
    const testSubject = `[TEST] Brevo Integration Verification`
    const testHtml = `
      <div style="background-color: #050505; color: #c8c0b0; font-family: 'Courier New', Courier, monospace; padding: 24px; border: 1px solid #1a4a1a; max-width: 640px; margin: 0 auto;">
        <h2 style="color: #4aff4a; border-bottom: 2px solid #1a4a1a; padding-bottom: 10px; margin-top: 0;">
          PROJECT NULL // BREVO TEST
        </h2>
        <p style="color: #b8862a; font-weight: bold;">
          ⚡ CONNECTION VERIFICATION SUCCESSFUL ⚡
        </p>
        <p>
          If you are reading this email, the Brevo integration is working perfectly.
        </p>
        <p style="margin: 16px 0; line-height: 1.5; color: #8a8070;">
          Timestamp: ${new Date().toISOString()}<br />
          Sender: ${process.env.EMAIL_FROM || 'onboarding@brevo.com (Default)'}<br />
          API Key Loaded: ${process.env.BREVO_API_KEY ? 'YES (Starts with ' + process.env.BREVO_API_KEY.substring(0, 6) + '...)' : 'NO (Simulator Fallback Active)'}
        </p>
...
PROJECT NULL // BREVO TEST
----------------------------------------------------------------------
If you are receiving this, the Brevo integration works!

Timestamp: ${new Date().toISOString()}
Sender: ${process.env.EMAIL_FROM || 'onboarding@brevo.com (Default)'}
API Key Loaded: ${process.env.BREVO_API_KEY ? 'YES' : 'NO (Simulator Fallback Active)'}
    `.trim()
    ...
    return NextResponse.json({
      ...
      message: delivery.success
        ? (delivery.method === 'brevo' ? 'Email sent successfully via Brevo API!' : 'Email logged to local simulation log (no Brevo API key configured).')
